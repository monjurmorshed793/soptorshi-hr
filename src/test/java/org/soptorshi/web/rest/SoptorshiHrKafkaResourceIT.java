...
import org.soptorshi.SoptorshiHrApp;
import org.soptorshi.config.SecurityBeanOverrideConfiguration;
...
@SpringBootTest(classes = {SecurityBeanOverrideConfiguration.class, SoptorshiHrApp.class})
@DirtiesContext(classMode = DirtiesContext.ClassMode.BEFORE_CLASS)
public class SoptorshiHrKafkaResourceIT {
    ...
    @Autowired
    private SoptorshiHrKafkaProducer producer;

    @Autowired
    private SoptorshiHrKafkaConsumer consumer;
    ...
    @BeforeEach
    public void setup() {
        SoptorshiHrKafkaResource kafkaResource = new SoptorshiHrKafkaResource(producer);
    ...
    public void producedMessageHasBeenConsumed() throws Exception {
        restMockMvc.perform(post("/api/soptorshi-hr-kafka/publish?message=test"))
